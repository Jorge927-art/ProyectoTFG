...
class ProfileControllerTest {
        ...
        @Test
        @DisplayName("POST /api/v1/profile/avatar - Debe devolver 400 si el servicio de almacenamiento rechaza el archivo por validación")
        void uploadAvatar_ValidationError() throws Exception {
                MockMultipartFile file = new MockMultipartFile(
                                "file", "avatar.jpg", MediaType.IMAGE_JPEG_VALUE, "imagenes_bytes".getBytes());
                ...
                when(authentication.getName()).thenReturn("luis");
                when(userRepository.findByUsername("luis")).thenReturn(Optional.of(mockUser));
                when(fileStorageService.storeFile(any(), eq("avatars")))
                ...
                mockMvc.perform(multipart("/api/v1/profile/avatar")
                                ...
                                .principal(authentication))
                                .andExpect(status().isBadRequest())
                                .andExpect(jsonPath("$.error").value("Formato no soportado"));
        }

        @Test
        @DisplayName("PUT /api/v1/profile/update - Debe crear un perfil extendido si aún no existe")
        void updateProfileData_CreatesProfileWhenMissing() throws Exception {
                String dtoJson = "{"
                                + "\"email\":\"nuevo_email@test.com\","
                                + "\"phoneNumber\":\"699999999\","
                                + "\"homeAddress\":\"Nueva Avenida 456\""
