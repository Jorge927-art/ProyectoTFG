...
public class TeacherAcademicGradingIntegrationTest {
    ...
    public void alRegistrarNota_DebeCalcularYReflejarLaMediaEnElExpediente() throws Exception {
        ...
        Mockito.when(enrollmentRepository.isInstructorAuthorizedForEnrollment(fakeEnrollmentId, "profesor_test"))
                .thenReturn(true);
        ...
        // aritméticas
        Mockito.when(userRepository.findActiveStudentsByCourseId(courseId))
                .thenReturn(Collections.singletonList(alumnoSimulado));
        Mockito.when(courseGradeRepository.getGroupAverageScore(courseId)).thenReturn(9.5);
        Mockito.when(courseGradeRepository.getIndividualStudentAverageScore(courseId, alumnoSimulado.getUser_id()))
                .thenReturn(9.5);
