...
export const UserScrollList = () => {
    ...
    return (
        <GenericCard className="w-full">
            {/* 1. MAQUETACIÓN DEL ENCABEZADO (Nativa, controlable y sin propiedades rígidas) */}
            <div className="mb-4">
            ...
            {/* 2. CUERPO DE LA UTILIDAD (El scroll propiamente dicho) */}
            {loading ? (
                    ...
                    {users.map((item) => (
                        <div key={item.userId} className="flex items-center justify-between p-2.5 rounded-lg border border-slate-50 hover:bg-slate-50/50 transition-all shadow-2xs">
                            <div className="flex items-center gap-2.5">
                                <div className="bg-slate-100 p-1.5 rounded-full text-slate-500"><User size={16} /></div>
                                <div>
