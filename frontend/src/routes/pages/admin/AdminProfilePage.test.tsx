...
describe('AdminProfilePage - Suite de Pruebas Unitarias Completa', () => {
    it('debe orquestar e integrar la jerarquía estructural correcta de la página de administración', () => {
        render(<AdminProfilePage />);

        // 1. Verificar la inicialización del layout contenedor de administración
        const dashboardLayout = screen.getByTestId('mock-dashboard-layout');
        expect(dashboardLayout).toBeInTheDocument();

        // 2. Verificar que la configuración del perfil se renderiza en su interior
